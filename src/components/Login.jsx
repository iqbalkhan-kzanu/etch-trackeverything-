...
function TeamIcon({ type, className }) {
  ...
  if (type === 'gauge') return (<svg {...common}><path d="M12 12l4-3" /><circle cx="12" cy="12" r="8" /><path d="M8 15a5 5 0 0 1 8 0" /></svg>)
  if (type === 'wind') return (<svg {...common}><path d="M3 8h11a3 3 0 1 0-3-3" /><path d="M3 13h15a3 3 0 1 1-3 3" /><path d="M3 18h8" /></svg>)
  if (type === 'flask') return (<svg {...common}><path d="M9 3h6" /><path d="M10 3v6l-5.5 9.5A2 2 0 0 0 6.2 21h11.6a2 2 0 0 0 1.7-2.5L14 9V3" /><path d="M8 15h8" /></svg>)
  if (type === 'droplet') return (<svg {...common}><path d="M12 3s6 6.5 6 11a6 6 0 0 1-12 0c0-4.5 6-11 6-11z" /></svg>)
  if (type === 'bolt') return (<svg {...common}><path d="M13 3 5 14h6l-1 7 8-11h-6l1-7z" /></svg>)
  return null
}
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2 font-sans">
      <div className="relative bg-ink text-white hidden lg:flex flex-col justify-between p-12 overflow-hidden">
        ...
        <p className="relative font-mono text-[11px] text-white/40">New Joinee Pilot · 5 Teams · 34 Engineers</p>
      </div>
      ...
      <div className="relative bg-canvas flex items-center justify-center px-6 py-16 overflow-hidden">
        ...
        <div className="relative w-full max-w-md">
              ...
              <div className="grid grid-cols-2 gap-3">
                {TEAMS.map((t) => {
                  ...
                  return (
                    ...
                    >
                      <div className="w-9 h-9 rounded-lg flex items-center justify-center mb-3" style={{ backgroundColor: meta.color, color: '#FFFFFF' }}>
                        <TeamIcon type={meta.icon} className="w-5 h-5" />
                      </div>
                      <p className="font-mono text-[10px] uppercase tracking-wider text-ink-muted">{ROSTER[t].length} members</p>
          ...
          {step === 'name' && (
            <>
              <button onClick={() => setStep('team')} className="font-mono text-xs uppercase tracking-wider text-ink-muted hover:text-ink mb-4 flex items-center gap-1">← Change team</button>
              <div className="flex items-center gap-2 mb-2">
                <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ backgroundColor: TEAM_META[team].color, color: '#FFFFFF' }}>
